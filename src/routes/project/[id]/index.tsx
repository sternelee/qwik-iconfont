  ...
  async ({ params, platform, request }): Promise<ProjectLoadResult> => {
    ...
    if (session) {
      ...
      const iconsResult = await db
        ...
        .orderBy(icons.sort_order, icons.created_at);
...
export default component$(() => {
  ...
  const validSort = ["name", "time", "unicode", "custom"] as const;
  const sortParam = loc.url.searchParams.get("sort");
  const sortBy = useSignal<"name" | "time" | "unicode" | "custom">(
    validSort.includes(sortParam as any)
      ? (sortParam as "name" | "time" | "unicode" | "custom")
      : "custom",
  );
  ...
  const draggedId = useSignal<number | null>(null);
  const dragOverId = useSignal<number | null>(null);
...
${classes}`;
  ...
  return (
    <div class="hero-gradient min-h-screen">
        ...
        <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="dropdown dropdown-end md:hidden">
              ...
              <ul
                tabIndex={0}
                class="dropdown-content clay-card z-[1] mt-2 w-44 p-2"
              >
                <li>
                  <button
          ...
          <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-3">
              {[
                ...
                {
                  label: "Unicode",
                  value: hasUnicodeCount,
                  icon: "M9.5 3A6.5 6.5 0 0 0 3 9.5c0 4.1 2.97 7.5 6.84 7.5h.67c.3 0 .58-.18.7-.45l1.18-2.63c.13-.28.4-.46.7-.46h2.02c.3 0 .57.18.7.46l1.18 2.63c.12.27.4.45.7.45h.67C17.03 17 20 13.6 20 9.5A6.5 6.5 0 0 0 13.5 3h-4z",
                },
                {
                  label: "标签",
                  value: allTags.value.length,
                  icon: "M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z",
                },
                {
                  label: "已选",
                  value: selectedIds.ids.size,
                  icon: "M22 11.08V12a10 10 0 1 1-5.93-9.14",
                },
                {
                  label: "收藏",
                  value: project.favorites_count || 0,
                  icon: "M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z",
                },
              ].map((stat) => (
                <div
                  key={stat.label}
                  class="clay-inset flex items-center gap-2.5 px-4 py-2.5"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
      ...
      {/* ── Icons Grid ────────────────────────────────────────── */}
      <div class="mx-auto max-w-7xl px-4 pt-5 pb-12 sm:px-6">
        {/* Stats strip */}
        {icons.list.length > 0 && (
          ...
          >
            ...
            {displayList.map((icon, idx) => (
              <div
                ...
                onDragOver$={(ev: any) => {
                  ev.preventDefault();
                  dragOverId.value = icon.id;
                }}
                onDragLeave$={() => {
                  if (dragOverId.value === icon.id) dragOverId.value = null;
                }}
                onDrop$={async (ev: any) => {
                  ev.preventDefault();
                  ...
                  if (!fromId || fromId === toId || sortBy.value !== "custom")
                    return;
                  const fromIdx = icons.list.findIndex(
                    (i: any) => i.id === fromId,
                  );
                  const toIdx = icons.list.findIndex((i: any) => i.id === toId);
                  if (fromIdx === -1 || toIdx === -1) return;
                  const newList = [...icons.list];
                  const [removed] = newList.splice(fromIdx, 1);
                  newList.splice(toIdx, 0, removed);
                  icons.list = newList;
                  const orderedIds = newList.map((i: any) => i.id);
                  await fetch(`/api/projects/${loc.params.id}/icons/reorder`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ iconIds: orderedIds }),
                  });
          ...
          <div class="clay-card animate-modal mx-4 max-w-lg">
            ...
            <form
              ...
              onSubmit$={async (ev: any) => {
                ...
                project.visibility = fd.get("visibility") as string;
                showSettings.value = false;
              ...
              <div class="mt-4 flex items-center justify-center gap-2">
                {[
                  ...
                  <button
                    key={c}
                    type="button"
                    class={`h-7 w-7 rounded-full border-2 transition-transform hover:scale-110 ${previewColor.value === c ? "scale-110 border-rose-400" : "border-transparent"}`}
                  ...
                  <div class="flex items-center gap-1">
                    {[
                      ...
                      <button
                        key={c}
                        type="button"
                        class={`h-4 w-4 rounded-full border-2 transition-transform hover:scale-110 ${previewColor.value === c ? "scale-110 border-rose-400" : "border-transparent"}`}
                        style={{ backgroundColor: c }}
                        onClick$={() => (previewColor.value = c)}
                      />
                    ))}
                ...
                >
                  ...
                  <div class="form-control mb-4">
                    ...
                    {showSVGSource.value && (
                      <textarea
                        name="content"
                        class="input-clay h-32 w-full px-4 py-2.5 font-mono text-xs"
              ...
              {codeMode.value === "fontclass" && selectedIds.ids.size > 0 && (
                <div class="mb-4 rounded-2xl bg-gradient-to-br from-rose-50 to-pink-50 p-4">
                  ...
                  {fontPreviewCSS.value && (
                    <style>{fontPreviewCSS.value}</style>
                  )}
                    ...
                    {icons.list
                      ...
                      .map((icon) => (
                        <div
                          ...
                          class="flex flex-col items-center gap-1"
                        >
                          <svg class="h-6 w-6 text-blue-500" aria-hidden="true">
                            <use href={`#${project.prefix}${icon.name}`} />
                          </svg>
                          <span class="text-[10px] text-blue-400/60">
                            {icon.name}
                          </span>
                        </div>
                      ))}
                      ...
                      .map((icon, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span class="text-[10px] text-emerald-400/60">
                              {icon.name}
                            </span>
                          </div>
                        );
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="form-control">
                    ...
                    <input
                      ...
                      onInput$={(ev: any) =>
                        (renameForm.find = ev.target.value)
                      }
                    />
                  </div>
                  <div class="form-control">
              ...
              <div class="flex justify-end gap-3">
                ...
                <button
                  ...
                  onClick$={async () => {
                    ...
                    const newTags = batchTagForm.tags
                      .split(",")
                      .filter(Boolean);
